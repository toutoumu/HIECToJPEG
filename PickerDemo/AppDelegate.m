...
- (BOOL)application:(UIApplication *) application didFinishLaunchingWithOptions:(NSDictionary *) launchOptions
{
    ...
    // 在沙盒中创建相册
    [TTFIleUtils createAlbum:@"Video"];
    [TTFIleUtils createAlbum:@"Deleted"];
    [TTFIleUtils createAlbum:@"Beauty"];
    [TTFIleUtils createAlbum:@"Decrypted"];
...
-(void)applicationDidEnterBackground:(UIApplication *)application{
    ...
    UIViewController *controller = [self.navController.viewControllers objectAtIndex:self.navController.viewControllers.count - 1];
    if (controller != nil ){
...
-(void)applicationDidBecomeActive:(UIApplication *)application{
    ...
    if (controller != nil && [controller isKindOfClass: [CameraViewController class]]){
        CameraViewController *camereViewController = (CameraViewController*)controller;
        // 如果没有弹出密码输入页面
        if ([camereViewController presentedViewController] == nil) {
...
-(void) initRoot{
    ...
    //[TTFIleUtils setPassword:@"111111"];// 全局密码缓存密码
