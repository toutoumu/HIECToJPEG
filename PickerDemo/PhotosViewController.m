...
- (void)viewDidLoad{
    ...
    _galleryViewController =  [mainStoryboard instantiateViewControllerWithIdentifier:@"GalleryViewController"];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 1) {
        if ([self.selectedAssetsURLs count] >0) {
            for (int i = 0; i<[self.selectedAssetsURLs count] ; i++) {
                //[TTFIleUtils deletePhotoByURL:[self.selectedAssetsURLs objectAtIndex:i]];
                NBUAsset *asset = [self.selectedAssets objectAtIndex:i];
                [asset delete:nil];
            }
            [self update];
        }
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex{
    NSLog(@"Button %li",  (long)buttonIndex);
    switch (buttonIndex) {
        case 0://select all
            [self selectAll:nil];
            isScan = NO;
